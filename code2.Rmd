...
model_3<-lm(BPSysAve~BMI+
              ...
              BMI*Race1 + Age*Gender + Gender*Race1 + Diabetes*DirectChol + Diabetes*AlcoholYear + Race1*DirectChol, data=dat)
summary(model_3)
```
...
```{r}
#AlcoholYear*Diabetes
interact_plot(model_3, pred = AlcoholYear, modx = Diabetes)
```

```{r}
#DirectChol*Diabetes
interact_plot(model_3, pred = DirectChol, modx = Diabetes)
```

```{r}
#Race*DirectChol
interact_plot(model_3, pred = DirectChol, modx = Race1)
```
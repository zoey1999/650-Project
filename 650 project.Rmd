...
interaction.plot(x.factor = comp_cases$PhysActiveDays, 
                 trace.factor = comp_cases$Gender,
                 response = comp_cases$BPSysAve)
```


```{r}
model_4<-lm(BPSysAve~BMI+Age+Gender+Race1+Education+DirectChol+Diabetes+PhysActiveDays+AlcoholYear+SmokeNow+Age*Gender+BMI*Race1+DirectChol*Diabetes+Gender*Race1,data=comp_cases)
summary(model_4)
